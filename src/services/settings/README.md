...
- **Şema v2 & Migration:** `UserSettings` şu gruplardan oluşur: `sound`, `controls`, `graphics`, `general` (+ `language`, `theme`). İlk açılışta veya v1 kayıtta dağınık eski anahtarlar (`lang`, `know_and_conquer_game_theme`, `soundMuted`, `hapticsEnabled`, `motionTier`) `legacyKeys.ts` ile taranıp yeni yapıya taşınır; kayıttaki değerler her zaman önceliklidir. Dil, tema ve ses kapalı bilgisi eski okuyucular için eski anahtarlara da yazılır (dual-write).
...
- **İnce adaptörler:** `haptics.ts`, `motionTier.ts` değerlerini bu servisten okur (dış API'leri ve otomatik karar mantıkları değişmedi).
