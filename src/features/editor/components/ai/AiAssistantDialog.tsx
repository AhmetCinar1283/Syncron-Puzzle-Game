...
const TABS: { id: AiTab; label: string; color: string; activeBg: string }[] = [
  { id: 'generator', label: '🤖 Prompt Generator', color: '#00c4ff', activeBg: 'rgba(0,196,255,0.08)' },
  { id: 'curriculum', label: '📚 Müfredat Düzenleyici', color: '#00ff88', activeBg: 'rgba(0,255,136,0.08)' },
  { id: 'templates', label: '📝 Şablonlar', color: '#a78bfa', activeBg: 'rgba(167,139,250,0.08)' },
  { id: 'rules', label: '⚙️ Oyun Kuralları (Referans)', color: '#fbbf24', activeBg: 'rgba(251,191,36,0.08)' },
];
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid rgba(30,58,95,0.3)', paddingBottom: 2, gap: 4 }}>
          {TABS.map((tab) => {
            ...
            return (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ padding: '6px 12px', fontSize: 11, fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase', background: active ? tab.activeBg : 'transparent', border: 'none', borderBottom: `2px solid ${active ? tab.color : 'transparent'}`, color: active ? tab.color : '#475569', cursor: 'pointer' }}>
                {tab.label}
              </button>
            );
          })}
