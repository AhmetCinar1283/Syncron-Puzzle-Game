    ...
    <div style={{ display: 'flex', gap: 20, flex: 1, overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1.2, display: 'flex', flexDirection: 'column', gap: 10, borderRight: '1px solid rgba(30,58,95,0.3)', paddingRight: 16, overflowY: 'auto' }}>
        <div style={{ display: 'flex', gap: 8 }}>
          <NBtn active={promptMode === 'create'} onClick={() => setPromptMode('create')} color="#00ff88" style={{ flex: 1, padding: '4px 0', fontSize: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
            <GameIcon name="plus" size={11} /> Seviye Üret (Create)
          </NBtn>
          <NBtn active={promptMode === 'improve'} onClick={() => setPromptMode('improve')} color="#fbbf24" style={{ flex: 1, padding: '4px 0', fontSize: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
            <GameIcon name="tools" size={11} /> Seviye İyileştir (Improve)
          </NBtn>
        </div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 180 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
            ...
            <button onClick={handleCopyPrompt} style={{ background: copied ? 'rgba(0,255,136,0.15)' : 'rgba(0,196,255,0.1)', border: `1px solid ${copied ? '#00ff88' : 'rgba(0,196,255,0.3)'}`, color: copied ? '#00ff88' : '#00c4ff', borderRadius: 4, padding: '2px 10px', fontSize: 10, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
              {copied ? (
                <>
                  <GameIcon name="check" size={11} /> Kopyalandı!
                </>
              ) : (
                <>
                  <GameIcon name="clipboard" size={11} /> Promptu Kopyala
                </>
              )}
            </button>
          </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <NBtn onClick={handleVerifyJson} color="#00c4ff" active style={{ flex: 1, padding: '7px 0', fontSize: 11, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
            <GameIcon name="lightning" size={12} /> Çözümü Simüle Et (Solver)
          </NBtn>
          <NBtn disabled={validationResult.status === 'error' || !aiJsonInput.trim()} onClick={handleImportToEditor} color={validationResult.status === 'unsolvable' ? '#fbbf24' : '#00ff88'} active={validationResult.status !== 'error' && !!aiJsonInput.trim()} style={{ flex: 1, padding: '7px 0', fontSize: 11, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
            <GameIcon name="import" size={12} /> Editöre Aktar (Import)
          </NBtn>
        </div>
        ...
        <div style={{ minHeight: 70 }}>
          {validationResult.status === 'success' && (
            <div style={{ background: 'rgba(0,255,136,0.06)', border: '1px solid rgba(0,255,136,0.3)', borderRadius: 8, padding: '8px 12px' }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#00ff88', marginBottom: 2, display: 'flex', alignItems: 'center', gap: 4 }}>
                <GameIcon name="check" size={12} /> BAŞARILI: Çözülebilir! ({validationResult.moves} Hamle)
              </div>
          ...
          {validationResult.status === 'unsolvable' && (
            <div style={{ background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.3)', borderRadius: 8, padding: '8px 12px', fontSize: 11, color: '#fbbf24' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                <GameIcon name="warning" size={12} /> <strong>UYARI: BULMACA ÇÖZÜLEMEDİ (YİNE DE İÇE AKTARILABİLİR)</strong>
              </div>
          ...
          {validationResult.status === 'error' && (
            <div style={{ background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 8, padding: '8px 12px', fontSize: 11, color: '#ef4444' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                <GameIcon name="error" size={12} /> <strong>HATA</strong>
              </div>
              <div style={{ fontSize: 10, color: '#94a3b8', marginTop: 2 }}>{validationResult.errorMsg}</div>
            </div>
          )}
