...
/**
 * Maskotun mini senaryoları (ana sayfa, seviyeler… her yerde ortak). Süreler `mascot/emotes.ts`
 * sürelerine göre boşluklu: ifadeler birbirine binmez, arka arkaya spam olmaz.
 */
export const SCENES: MascotScene[] = [
  {
    id: 'hello',
...
export const SINGLES: EmoteName[] = ['happy', 'wink', 'love', 'surprised', 'blink'];

const SINGLE_CHANCE = 0.25;

export type ShowPick =
  | { kind: 'scene'; scene: MascotScene }
 ...
 * `scenes`/`singles` verilmezse ortak katalog kullanılır.
 */
export function pickShow(
  rand: () => number,
  lastId: string | null,
  scenes: MascotScene[] = SCENES,
  singles: EmoteName[] = SINGLES,
): ShowPick {
  if (rand() < SINGLE_CHANCE) {
    const emote = singles[Math.floor(rand() * singles.length)];
    return { kind: 'single', emote, duration: 1400 };
  }
  const pool = scenes.filter(s => s.id !== lastId);
